            ...
            <div class="col-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    ...
                    <form class="forms-sample" action="{{route('charges.update', $charge)}}" method="POST">
                      @csrf
                      @method('PUT')
                      
                      <div class="form-group ">
                        <label for="label">Désignation</label>
                        <input type="text" class="form-control" id="label" name="label" value="{{$charge->label}}" placeholder="Désignation">
                      </div>
                        ...
                        <label for="id_ag">Agence</label>
                          <select class="form-control" id="id_ag" name="id_ag">
                            @foreach ($agencies as $agency)
                                <option value="{{$agency->id}}" {{($agency->id == $charge->id_ag)? "selected" : ""}}>{{$agency->code_ag}}</option>
                            @endforeach
                          </select>
                        </div>
                        @endif
                        <div class="form-group ">
                            ...
                            <select class="form-control" id="id_cat" name="id_cat">
                                @foreach ($charges_cats as $cat)
                                    <option value="{{$cat->id}}" {{($cat->id == $charge->id_cat)? "selected" : ""}}>{{$cat->name}}</option>
                                @endforeach
                            </select>
                        </div>
                      @if (Auth::user()->type == 'Admin')
                      <div class="form-group ">
                          ...
                          <input type="date" class="form-control" id="date" value="{{$charge->date}}" name="date" placeholder="Date">
                      </div>
                      @endif
                      <div class="form-group">
                        <label for="montant">Monatant</label>
                        <input type="number" class="form-control" id="montant" name="montant" value="{{$charge->montant}}" placeholder="Monatant">
                      </div>
                      @if (Auth::user()->type == 'Admin')
                      <div class="form-group">
                            ...
                            <select class="form-control" id="id_user" name="id_user">
                                @foreach ($users as $user)
                                    <option value="{{$user->id}}" {{($user->id == $charge->id_user)? "selected" : ""}}>{{$user->name}}</option>
                                @endforeach
                            </select>
                      </div>
                      @endif
                      <div class="form-group">
                        ...
                        <textarea class="form-control" name="note"  id="exampleTextarea1" rows="4">{{$charge->note}}</textarea>
                      </div>
